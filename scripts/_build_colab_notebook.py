...
cells.append(code([
    ...
    "\n",
    "# Training dirs use _jitter suffix so the spatial-randomization rebuild\n",
    "# doesn't shadow the older fixed-framing checkpoints on Drive. Test dirs\n",
    "# stay at the original names so 'in-distribution' baseline numbers are\n",
    "# directly comparable across runs.\n",
    "DATASETS = {\n",
    "    'boxes': {\n",
    "        'script':       'scripts/build_box_data.py',\n",
    "        'train_dir':    'data/train64_box_jitter',\n",
    "        'test_dir':     'data/test64_box',\n",
    "        'out_dir':      'checkpoints/ar_box64_jitter',\n",
    "        'extra_args':   ['--randomize-framing'],\n",
    "        'reference':    'boxes -- Stage 1 ship (no-jitter): F1=0.980, exact=0.932',\n",
    "    },\n",
    "    'cylinders': {\n",
    "        'script':       'scripts/build_cylinder_data.py',\n",
    "        'train_dir':    'data/train64_cyl_jitter',\n",
    "        'test_dir':     'data/test64_cyl',\n",
    "        'out_dir':      'checkpoints/ar_cyl64_jitter',\n",
    "        'extra_args':   ['--randomize-framing'],\n",
    "        'reference':    'cylinders -- Stage 2 ship (no-jitter): F1=0.995, exact=0.988',\n",
    "    },\n",
    "    'mixed': {\n",
    "        'script':       'scripts/build_mixed_3d_data.py',\n",
    "        'train_dir':    'data/train64_mixed_3d_jitter',\n",
    "        # eval uses the PURE held-out splits separately (Gate 4: both have to pass)\n",
    "        'test_dir':     'data/test64_box',\n",
    "        'out_dir':      'checkpoints/ar_mixed_3d_jitter',\n",
    "        'extra_args':   ['--fraction-cyl', '0.5', '--randomize-framing'],\n",
    "        'reference':    'mixed boxes+cylinders + spatial jitter -- the reality-probe fix run',\n",
    "    },\n",
    "}\n",
