...
cells.append(code([
    ...
    "\n",
    "target = 0.7\n",
    "if r_greedy['mean_f1'] >= target:\n",
    "    print(f'OK  F1 {r_greedy[\"mean_f1\"]:.3f} >= {target}: A3 succeeded; Stage 2 (ellipses) is unblocked.')\n",
    "elif r_greedy['mean_f1'] > 0.282:\n",
    "    print(f'PARTIAL  F1 {r_greedy[\"mean_f1\"]:.3f} > 0.282 (beats set predictor) but < {target} target.')\n",
    "    print('   First try: more epochs (cfg.epochs *= 2) -- training may not have converged.')\n",
    "else:\n",
    "    print(f'UNEXPECTED  F1 {r_greedy[\"mean_f1\"]:.3f} <= 0.282. See docs/a3-gpu-handoff.md rubric.')\n",
]))
